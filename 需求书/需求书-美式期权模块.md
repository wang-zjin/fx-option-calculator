...
| **共享输入** | 货币对、当前日期、起息日、到期日、交割日、即期、执行价、本币利率、外币利率、名义本金、买卖方向、期权类型（看涨/看跌）、隐含波动率；**T1** = (到期日−当前日期)/365 用于树期限，**T2** = (交割日−起息日)/365 用于到期 payoff 折现（与 Vanilla 一致）。 |
| **计算复用** | 直接调用现有 `models/american` 的 `priceAmerican(params, optionType)`，传入 `AmericanParams`；不新增定价算法，仅新增前端页面与参数组装。 |
| **类型与接口** | `AmericanParams` 继承 `GKParams`（S, K, **T1, T2**, r_d, r_f, sigma），并增加 steps、treeType；`AmericanResult` 含 price、earlyExercisePremium、delta、gamma、vega、theta、earlyExerciseBoundary（看跌可选）。 |
...
### 3.2 双期限 T1、T2（与外汇期权到期日/交割日一致）

外汇期权存在**到期日**与**交割日**两个时间，美式期权定价需**同时使用两个期限**，与 Vanilla 口径一致：

| 符号 | 含义 | 计算式 | 用途 |
|------|------|--------|------|
| **T1** | 到期期限 | (到期日 − 当前日期) / 365 | **树构造**：期权存续期、每步 Δt = T1/n、向后归纳至今日。 |
| **T2** | 折现期限 | (交割日 − 起息日) / 365 | **到期 payoff 折现**：到期时 payoff 在**交割日**支付，需折现到**起息日**口径，与 Vanilla 报价一致。 |

**实现约定**：在树模型的**最后一层（到期日节点）**，节点价值 = 内在价值 × e^{−r_d·T2}（将交割日 payoff 折现到起息日），再按树向后归纳至根节点。这样得到的美式期权价格与 Vanilla 同为「起息日口径」；Premium = 期权价格 × 名义本金。若当前计算层尚未使用 T2，需在美式树中补充上述最后一层折现。

### 3.3 定价模型（与 models/american 一致）

- **二叉树**：Cox-Ross-Rubinstein (CRR)，步数 n，每步 Δt = T1/n，漂移 r_d − r_f，贴现 r_d；**到期层**使用 T2 折现 payoff。
- **三叉树**：每步三种状态（上/平/下），概率由一、二阶矩匹配；贴现与美式行权判断同二叉树；**到期层**同样使用 T2 折现 payoff。
...
**双期限 T1、T2**：  
- **T1** = (到期日 − 当前日期) / 365，由前端传入 `AmericanParams.T1`，用于树构造（期权存续期）。  
- **T2** = (交割日 − 起息日) / 365，由前端传入 `AmericanParams.T2`，用于到期 payoff 折现（交割日 → 起息日），与 Vanilla 报价口径一致。  
前端从日期计算 T1、T2 后一并传入计算层；计算层在树的最后一层使用 T2 折现 payoff。
...
2. **参数组装**：前端将表单转为 `AmericanParams`：  
   - 从共享输入得到 S, K, **T（= T1）**、**T2**、r_d, r_f, sigma,（T1、T2 由日期计算）；  
   - 从美式专用输入得到 steps（默认 200）、treeType（默认 `crr`）；  
   - **T2 必传**，用于到期层 payoff 折现（交割日 → 起息日），与 Vanilla 双期限口径一致。  
...
- **AmericanParams**：继承 GKParams（S, K, **T1**, **T2**（交割日−起息日，用于到期 payoff 折现）, r_d, r_f, sigma）；扩展：`steps?: number`（默认 200），`treeType?: 'crr' | 'trinomial'`（默认 `crr`）。美式定价需**同时传入 T1、T2**，与外汇期权到期日/交割日两个时间一致。  
